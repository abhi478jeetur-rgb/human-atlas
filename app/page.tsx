...
import {Activity,ArrowUpRight,ChevronRight,Focus,Info,Keyboard,Layers3,Pause,RotateCcw,RotateCw,Search,Volume2,X} from 'lucide-react';
...
export default function Home(){
 ...
 useEffect(()=>{
  const key=(e:KeyboardEvent)=>{
   ...
   else if(e.key==='Escape'){setPanel(null);setDetails(false);setAbout(false);setHelp(false);}
  };
 ...
 return <main className="studio">
  ...
  <nav className="top-actions" aria-label="Explorer panels"><Button variant="ghost" className={panel==='search'?'active':''} onClick={()=>openPanel('search')} aria-label="Search anatomy"><Search size={18}/><span>Find a structure</span><kbd>/</kbd></Button><Button variant="ghost" className="icon-button" aria-label="Keyboard shortcuts" title="Keyboard shortcuts (?)" onClick={()=>{setDetails(false);setPanel(null);setHelp(true);}}><Keyboard size={18}/></Button><Button variant="ghost" className="icon-button" aria-label="About this atlas" onClick={()=>{setDetails(false);setPanel(null);setAbout(true);}}><Info size={18}/></Button></nav>
