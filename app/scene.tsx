...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance',preserveDrawingBuffer:true});}catch{onError('This browser could not start the 3D viewer. Please try a browser with WebGL enabled.');return;}
  const takeSnapshot=()=>{
   if(!renderer||disposed)return;
   renderer.render(scene,camera);
   const dataUrl=renderer.domElement.toDataURL('image/png');
   const a=document.createElement('a');
   a.download=`human-atlas-${Date.now()}.png`;
   a.href=dataUrl;
   a.click();
  };
  window.addEventListener('atlas:screenshot',takeSnapshot);
  ...
  const surgicalLight=new T.DirectionalLight(0xdff0ff,3.2);surgicalLight.position.set(0,4.8,0.5);surgicalLight.visible=false;scene.add(surgicalLight);
  ...
  const cadaverTable=new T.Group();
  const tableTop=new T.Mesh(new T.BoxGeometry(0.78,0.035,1.96),new T.MeshStandardMaterial({color:0xa0abb5,metalness:0.85,roughness:0.22}));tableTop.position.set(0,-0.018,0);cadaverTable.add(tableTop);
  const tableRim=new T.Mesh(new T.BoxGeometry(0.82,0.05,2.0),new T.MeshStandardMaterial({color:0x75808b,metalness:0.92,roughness:0.18}));tableRim.position.set(0,-0.025,0);cadaverTable.add(tableRim);
  const tableBase=new T.Mesh(new T.CylinderGeometry(0.24,0.28,0.3,32),new T.MeshStandardMaterial({color:0x343a40,metalness:0.8,roughness:0.35}));tableBase.position.set(0,-0.17,0);cadaverTable.add(tableBase);
  cadaverTable.visible=false;scene.add(cadaverTable);
  ...
  const impulseUniforms={time:{value:0},active:{value:0}};
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    ...
    shader.uniforms.impulseTime=impulseUniforms.time;shader.uniforms.impulseActive=impulseUniforms.active;
    shader.vertexShader='attribute float partIndex; uniform sampler2D partState; uniform sampler2D selectionState; uniform float stateWidth; varying float partVisible; varying float partSelected; varying vec3 vWorldPos;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvec2 stateUv = vec2((partIndex + 0.5) / stateWidth, 0.5); vec4 state = texture2D(partState, stateUv); transformed += state.xyz; partVisible = state.w; partSelected = texture2D(selectionState, stateUv).r; vWorldPos = (modelMatrix * vec4(transformed, 1.0)).xyz;');
    shader.fragmentShader='varying float partVisible; varying float partSelected; varying vec3 vWorldPos; uniform float impulseTime; uniform float impulseActive;\n'+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (partVisible < 0.5) discard;');
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.42, 0.85, 0.78), partSelected * 0.75);\nif (impulseActive > 0.5) { float pulse = sin(vWorldPos.y * 16.0 - impulseTime * 14.0) * 0.5 + 0.5; diffuseColor.rgb += vec3(0.15, 0.85, 1.0) * pow(pulse, 4.0) * 2.5; }');
  ...
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'Could not load the anatomy.');}})();
    const fit=(view:string,extent=0,region:string='all')=>{
     const isCadaver=!!latest.current.cadaverMode;
     ...
     let targetY=isCadaver?0.82:extent>.1||mobile?.85:.68;
     let dist=isCadaver?(mobile?2.8:2.35):normalDistance;
     if(!isCadaver&&extent<.1){
      if(region==='head'){targetY=1.55;dist=mobile?1.2:.95;}
      else if(region==='thorax'){targetY=1.18;dist=mobile?1.6:1.3;}
      else if(region==='abdomen'){targetY=.88;dist=mobile?1.6:1.25;}
      else if(region==='pelvis'){targetY=.58;dist=mobile?1.4:1.15;}
     }
     const distance=extent>.1?T.MathUtils.lerp(normalDistance,Math.max(.2,atlasDistance),extent):dist;if(extent>.8)view='front';
     const direction=isCadaver?new T.Vector3(0.42,0.68,0.88).normalize():view==='front'?new T.Vector3(0,.02,1):view==='back'?new T.Vector3(0,.02,-1):view==='side'?new T.Vector3(1,.02,0):new T.Vector3(.35,.06,1).normalize();
     controls.target.set(extent>.1&&el.clientWidth>767?-packingWidth*.12:0,targetY,0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
    };
    const resize=()=>{layoutKey='';lastState=null;renderer.setPixelRatio(Math.min(devicePixelRatio,el.clientWidth<768||el.clientHeight<600?1.5:2));camera.aspect=el.clientWidth/el.clientHeight;camera.updateProjectionMatrix();renderer.setSize(el.clientWidth,el.clientHeight);fit(latest.current.view,amount,latest.current.region);};const observer=new ResizeObserver(resize);observer.observe(el);
   ...
   const animate=()=>{
    ...
    const changed=lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate||lastState?.cadaverMode!==s.cadaverMode||lastState?.dissectionDepth!==s.dissectionDepth;
    ...
    if(s.view!==lastView||s.reset!==lastReset||s.cadaverMode!==lastState?.cadaverMode){fit(s.view,amount,s.region);lastView=s.view;lastReset=s.reset;}
    if(moving&&!s.isolate)fit(amount>.5?'front':s.view,Math.max(0,(amount-.3)/.7),s.region);
    ...
    if(isolateKey!==lastIsolate||(s.isolate&&moving)){
     ...
     }else if(lastIsolate){camera.clearViewOffset();fit(s.view,amount,s.region);}
     lastIsolate=isolateKey;
    }
    const isCadaver=!!s.cadaverMode;
    cadaverTable.visible=isCadaver;
    surgicalLight.visible=isCadaver;
    if(isCadaver){
     renderer.setClearColor('#0e1217');
    ...
    if(skinMat&&skinMat instanceof T.MeshStandardMaterial){
     if(isCadaver&&s.dissectionDepth!==undefined){
      if(s.dissectionDepth<=15){
       skinMat.opacity=T.MathUtils.lerp(0.9,0.15,s.dissectionDepth/15);
       skinMat.transparent=true;
       skinMat.depthWrite=skinMat.opacity>0.45;
       skinMat.color.setHex(0xcab098);
      }else{
       skinMat.opacity=0;
      ...
      skinMat.opacity=0.1;
      skinMat.depthWrite=false;
      skinMat.color.setHex(0xba9b7d);
     }
    }
    impulseUniforms.time.value+=dt;
    impulseUniforms.active.value=s.impulseActive?1:0;
    if(s.impulseActive)dirty=true;
  ...
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(frame);window.removeEventListener('atlas:screenshot',takeSnapshot);observer.disconnect();controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();};
 },[atlas]);
